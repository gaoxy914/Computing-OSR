...
int main() {
	...
	for (double rho = 0.07; rho <= 0.1; rho += 0.01) {
		string log_path = "./logs/time_" + to_string(rho) + ".log";
		string result_path = "./results_1/result_" + to_string(rho) + ".csv";
		for (int i = 5; i <= 5; i++) {
			...
			for (size_t i = 0; i < round; ++i) {
				...
				double right = left + number / (10 * (rand() % 2 + 2));
			...
			outFile << number << ","
				<< sum / round << "," //<< max << "," << min << ","
				<< sum_online / round << ","// << max_online << "," << min_online << ","
				<< sum_sp / round << "," //<< max_sp << "," << min_sp << ","
				<< sum_online_sp / round << endl;// "," << max_online_sp << "," << min_online_sp << endl;
			outFile.close();
			now = time(0);
