...
//	size_t dirty = number - clean;
	size_t Id = 0, clean_count = 0, dirty_count = 0;
	data_t data;
	...
	while (Id < number) {
		size_t clean_Id = rand() % clean;
		pollute(Id++, clean_Id, &data);
		fwrite(&data, sizeof(data_t), 1, fp);
	}
